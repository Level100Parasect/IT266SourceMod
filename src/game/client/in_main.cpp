...
static	kbutton_t	in_firemode; //louis
...
void IN_FireModeDown( const CCommand &args ) {KeyDown(&in_firemode, args[1] );}//louis
void IN_FireModeUp( const CCommand &args ) {KeyUp(&in_firemode, args[1] );}//louis
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_FIREMODE, s_ClearInputState, &in_firemode, bResetState ); //louis
...
static ConCommand startfiremode("+firemode", IN_FireModeUp);//louis
static ConCommand endfiremode("-firemode", IN_FireModeDown);//louis
